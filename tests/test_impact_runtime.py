...
def run_hook(script, payload, *, cwd, tmpdir):
    env = dict(os.environ)
...
class ClaudeImpactRuntimeTests(unittest.TestCase):
    def test_major_scope_drift_blocks_once(self):
        with tempfile.TemporaryDirectory() as temp, tempfile.TemporaryDirectory() as state:
            ...
            session_id = "impact-" + uuid.uuid4().hex
            submit = run_hook(
                VERIFY,
                {
                    ...
                    "cwd": str(root),
                    "prompt": "Use high-agency-coding to update both areas",
