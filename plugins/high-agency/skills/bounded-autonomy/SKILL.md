...
description: Use when the user asks Claude to keep working autonomously, iterate until done, fix until tests pass, or run a bounded Ralph-style loop with objective or practically verifiable acceptance criteria.
---
...
Do not use more passes to compensate for weak reasoning. If the same underlying failure survives two evidence-based attempts, use the model/effort escalation policy from `high-agency-coding/references/model-routing.md` when available before asking for more iterations.
