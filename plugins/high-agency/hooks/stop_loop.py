...
def bdir(): return Path(tempfile.gettempdir())/"high-agency-claude-code"
def vdir(): return Path(tempfile.gettempdir())/"high-agency-claude-code-verification"
def bpath(turn): return bdir()/(hashlib.sha256(turn.encode()).hexdigest()[:24]+".json")
def vpath(payload):
    raw=str(payload.get("session_id") or payload.get("turn_id") or "default")
    safe=re.sub(r"[^A-Za-z0-9_.-]","_",raw)[:120]
    return vdir()/(safe+".json")
def load(p):
...
def main():
    ...
    msg=last_assistant_message(Path(transcript))
    if not msg:return 0
    cleanup(bdir())
    cleanup(vdir())
    bp=bpath(transcript)
    ...
    if vd.get("active") and code_files(pass_files) and vd.get("dirty_since_verification") and not vd.get("verification_guard_warned"):
        ...
        emit({"decision":"block","reason":"High Agency verification guard: relevant edits occurred after the last verification. Run the narrowest relevant targeted/affected check. Do not run the full suite unless risk justifies it."+suffix,"systemMessage":"High Agency: refresh targeted verification"})
        return 0
    if not marker and vd.get("active") and not vd.get("diff_review_seen") and not vd.get("diff_guard_warned"):
        ...
        if reasons:
            ...
            emit({"decision":"block","reason":"High Agency conditional diff review: "+"; ".join(reasons)+". Inspect only the focused final diff for touched files ("+preview+"), confirm no unrelated changes/API drift, then finish. Do not launch a reviewer subagent or broad branch review unless requested."+extra,"systemMessage":"High Agency: focused final diff review"})
            return 0
    if not marker:
        clear(bp)
        clear(vp)
    ...
    emit({"decision":"block","reason":reason,"systemMessage":f"High Agency continuation {count}/{lim}"})
    return 0
